...
| **Agent isolation** | Each agent in its own Docker container with per-agent git repo mounts, iptables rules, no shared state | Docker sandboxing for non-main sessions; shared-state sessions via JSONL transcripts |
...
| **Filesystem control** | Per-agent git repositories with kernel bind mounts — the mount set is the ACL; gateway-only git with per-session provenance commits | Standard Docker filesystem isolation |
...
| **Skills / plugins** | Skill files loaded at session start; mount-enforced -- undeclared repos are simply not mounted | ClawHub marketplace (bundled, managed, workspace skills) |
