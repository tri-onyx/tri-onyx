...
reading text, screenshots, scrolling, and listing accessible elements —
all driven by Playwright's get_by_role/get_by_label locators.
...
def _browser(args, url=None):
    """Open a browser page with viewport from args, yield it, then handle screenshot and cleanup."""
    ensure_browsers_installed()
    PROFILE_DIR.mkdir(parents=True, exist_ok=True)

    width = getattr(args, "width", None) or DEFAULT_WIDTH
    height = getattr(args, "height", None) or DEFAULT_HEIGHT
    wait = getattr(args, "wait", None) or "load"

    target_url = url
    if target_url is None:
        target_url = _load_last_url()
        if not target_url:
    ...
    with sync_playwright() as p:
        ...
        page.goto(target_url, wait_until=wait)
        _wait_for_htmx(page)
...
def cmd_navigate(args):
    ...
    with _browser(args, url=url) as page:
        _save_last_url(url)
    ...
    with _browser(args) as page:
        if args.role and args.name:
            ...
            try:
                ...
                print(f"Error: could not find {args.role} '{args.name}'", file=sys.stderr)
                sys.exit(1)
            print(f"Scrolled to {args.role} '{args.name}'")
        else:
            amount = args.amount
            ...
            label = "up" if amount < 0 else "down"
            print(f"Scrolled {label} {abs(amount)}px")
...
def main():
    ...
    # Shared options for all browser commands
    browser_parent = argparse.ArgumentParser(add_help=False)
    browser_parent.add_argument("--screenshot", "-s", metavar="PATH", help="Save screenshot after action")
    browser_parent.add_argument("-W", "--width", type=int, help=f"Viewport width (default: {DEFAULT_WIDTH})")
    browser_parent.add_argument("-H", "--height", type=int, help=f"Viewport height (default: {DEFAULT_HEIGHT})")

    # navigate
    p_nav = sub.add_parser("navigate", parents=[browser_parent], help="Navigate to a URL")
    ...
    # click
    p_click = sub.add_parser("click", parents=[browser_parent], help="Click an element by role and name")
    ...
    # fill
    p_fill = sub.add_parser("fill", parents=[browser_parent], help="Fill an input by role and name")
    ...
    # read
    p_read = sub.add_parser("read", parents=[browser_parent], help="Read text content from an element or the page")
    p_read.add_argument("role", nargs="?", help="ARIA role (optional)")
    p_read.add_argument("name", nargs="?", help="Accessible name (optional)")
    p_read.set_defaults(func=cmd_read)

    # screenshot
    p_ss = sub.add_parser("screenshot", parents=[browser_parent], help="Screenshot the current page")
    ...
    # scroll
    p_scroll = sub.add_parser("scroll", parents=[browser_parent], help="Scroll the page or to an element")
    p_scroll.add_argument("role", nargs="?", help="ARIA role of element to scroll to (optional)")
    p_scroll.add_argument("name", nargs="?", help="Accessible name of element to scroll to (optional)")
    p_scroll.add_argument("-d", "--direction", choices=["down", "up"], default="down", help="Scroll direction (default: down)")
    p_scroll.add_argument("-a", "--amount", type=int, default=500, help="Scroll distance in pixels (default: 500)")
    p_scroll.set_defaults(func=cmd_scroll)

    # list
    p_list = sub.add_parser("list", parents=[browser_parent], help="List accessible elements on the page")
    p_list.add_argument("--role", help="Filter by role")
    p_list.set_defaults(func=cmd_list)

    # wait
    p_wait = sub.add_parser("wait", parents=[browser_parent], help="Wait for text to appear on the page")
