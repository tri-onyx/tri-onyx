...
Risk profiles are loaded from a pre-exported JSON file generated by:
    docker run --rm -v $(pwd):/app -w /app tri-onyx-gateway:latest \
        mix tri_onyx.export_agent_risk 2>/dev/null > tmp/agent_risk.json
...
def load_risk_data() -> dict:
    ...
    if not RISK_JSON.exists():
        print(
            f"Warning: {RISK_JSON} not found — risk profiles will be omitted.\n"
            f"Generate it with:\n"
            f"  docker run --rm -v $(pwd):/app -w /app tri-onyx-gateway:latest "
            f"mix tri_onyx.export_agent_risk 2>/dev/null > tmp/agent_risk.json",
            file=sys.stderr,
        )
        return {}
    return json.loads(RISK_JSON.read_text())


RISK_BADGE_CSS = {
    "low": "tx-badge--risk-low",
    "medium": "tx-badge--risk-moderate",
    "moderate": "tx-badge--risk-moderate",
    "high": "tx-badge--risk-high",
    "critical": "tx-badge--risk-critical",
}
...
def generate_agent_page(meta: dict, body: str, risk: dict | None = None) -> str:
...
def generate_index(agents: list[dict], risk_data: dict | None = None) -> str:
    ...
    for a in sorted(agents, key=lambda x: x["name"]):
        ...
        lines.append(f"  <div class=\"tx-agent-card\" onclick=\"location.href='{name}/'\">" )
        lines.append(f'    <div class="tx-agent-card__name"><a href="{name}/">{name}</a></div>')
        if desc:
            lines.append(f'    <div class="tx-agent-card__desc">{desc}</div>')

        # Badges
        badge_parts = [f'<span class="tx-badge tx-badge--model">{short_model}</span>']

        if isinstance(network, list):
            badge_parts.append(f'<span class="tx-badge tx-badge--network-restricted">{len(network)} hosts</span>')
        elif network == "outbound":
            badge_parts.append('<span class="tx-badge tx-badge--network-outbound">outbound</span>')
        else:
            badge_parts.append('<span class="tx-badge tx-badge--network-none">no network</span>')

        if a.get("browser"):
            badge_parts.append('<span class="tx-badge tx-badge--browser">browser</span>')

        # Risk badge on card
        risk = risk_data.get(name)
        if risk:
            effective = risk["effective_risk"]
            css = RISK_BADGE_CSS.get(effective, "tx-badge--risk-low")
            badge_parts.append(f'<span class="tx-badge {css}">{effective} risk</span>')

        lines.append(f'    <div class="tx-badges">{"".join(badge_parts)}</div>')
        lines.append("  </div>")
...
def main():
    ...
    for path in definition_files:
        try:
            ...
            continue
        agents.append(meta)
        agent_risk = risk_data.get(meta["name"])
        pages[meta["name"]] = generate_agent_page(meta, body, risk=agent_risk)

    agent_names = [a["name"] for a in agents]
    index_content = generate_index(agents, risk_data=risk_data)
